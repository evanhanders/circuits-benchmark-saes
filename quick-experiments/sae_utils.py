...
def make_sae_lens_config(
    ...
    **kwargs
    ...
    cfg_input = dict(
        ...
        # SAE Parameters
        architecture = "standard", #or "gated"
        d_in = model.cfg.d_model,
        d_sae = None,
        b_dec_init_method = "geometric_median",
        expansion_factor = 4,
        activation_fn = "relu",  # relu, tanh-relu, topk
        activation_fn_kwargs = {},  # for topk
...
def make_topk_sae_lens_config(
    ...
    **kwargs
    ...
    cfg =  make_sae_lens_config(
        model, hook_name, hook_layer,
        activation_fn="topk",
    ...
    cfg.run_name = f"{cfg.d_sae}-topk-{k}-LR-{cfg.lr}-Tokens-{cfg.training_tokens:3.3e}"
    return cfg

def make_gated_sae_lens_config(
    model : HookedTransformer,
    hook_name: str, 
    hook_layer: int,
    **kwargs
    ...
    cfg.run_name = f"{cfg.d_sae}-gated-L1-{cfg.l1_coefficient}-LR-{cfg.lr}-Tokens-{cfg.training_tokens:3.3e}"
    return cfg
...
def train_sae(
    ...
    store = ActivationsStore.from_config(model, runner_cfg, override_dataset=dataset)
...
def make_token_df(model, tokens, len_prefix=5, len_suffix=1, extra_token_labels = {}):
    """
    Evan note: extra_token_labels is a dict with string labels containing torch tensors of the same shape as tokens.
    """
    ...
    prefix_list = []
    suffix_list = []
    out_extras = { k : [] for k in extra_token_labels.keys() }
    ...
    for b in range(tokens.shape[0]):
        ...
        for p in range(tokens.shape[1]):
            ...
            pos.append(p)
            label.append(f"{b}/{p}")
            for k in extra_token_labels.keys():
                out_extras[k].append(extra_token_labels[k][b][p].item())
